...
PersistentLayout persistent_layout(const SequencePlanImpl& plan) {
    ...
    if constexpr (Variant::supports_dflash) {
        if (plan.features.masked_draft()) {
            ...
            // Guarded because 27B sets full_layers = 0: without it the layout allocates a full
            // KV plane the model has no full-attention layer for, and dflash_context_impl's
            // `full.has_value() != (full_layers != 0)` check then rejects the runtime. Field names
            // are this fork's (`head_dim` + `storage`), not upstream's `layer_storage`.
