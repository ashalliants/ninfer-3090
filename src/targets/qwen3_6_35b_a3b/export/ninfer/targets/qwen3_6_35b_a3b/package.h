...
struct Package {
    ...
    // Pipeline split: plan one rank at a time. ownership decides which layers this rank uploads.
    [[nodiscard]] static LoadPlan plan_load(artifact::Binder& binder, const EngineOptions& options,
                                            WeightsProfile weights_profile,
    ...
    // One plan and one artifact per rank, paired by index with the split.
    [[nodiscard]] static std::unique_ptr<LoadedModel>
