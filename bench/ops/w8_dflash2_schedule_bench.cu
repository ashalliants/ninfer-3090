// Route-boundary sweep for the two W8 DFlash2 tables, on the 27B hot path.
//
...
// The exact shape is_dflash2_shape() admits in w8_attn_input_plan.cpp. Miss it by one and the
...
void sweep_attn_input(const ninfer::bench::SweepOptions& base) {
    ...
    std::vector<ninfer::bench::SweepEntry> schedules{
        {"small_t", make(&detail::w8_dflash2_attn_input_small_t_launch), 48},
        {"mma_r16_c64_k128", make(&detail::w8_dflash2_attn_input_mma_r16_c64_k128_launch), 0},
        {"mma_r32_c32_k128", make(&detail::w8_dflash2_attn_input_mma_r32_c32_k128_launch), 0},
        {"mma_r32_c64_k128", make(&detail::w8_dflash2_attn_input_mma_r32_c64_k128_launch), 0},
        {"mma_r32_c64", make(&detail::w8_dflash2_attn_input_mma_r32_c64_launch), 0},
        {"mma_r64_c128", make(&detail::w8_dflash2_attn_input_mma_r64_c128_launch), 0},
    };

    ninfer::bench::SweepOptions options = base;
    options.title                       = "w8 dflash2 attn_input (27B)";
    std::string routed;
    options.routed_name = [&routed](std::int32_t tokens) -> const char* {
        const detail::W8AttnInputProblem problem{kHidden,     kQueryRows, kKVRows,
                                                 kParentRows, kHidden,    tokens};
        routed = detail::w8_attn_input_schedule_name(
            detail::w8_attn_input_resolve_plan(problem).schedule);
        return routed.c_str();
    };
    options.public_op = [&](std::int32_t tokens, cudaStream_t stream) {
        ...
        Tensor k(key.p, DType::BF16, {kKVRows, tokens});
        Tensor v(value.p, DType::BF16, {kKVRows, tokens});
        detail::w8_attn_input_dispatch(x, packed.weight, q, k, v, stream);
    };
...
void sweep_linear_swiglu(const ninfer::bench::SweepOptions& base) {
    ...
    std::vector<ninfer::bench::SweepEntry> schedules{
        {"small_t", make(&detail::w8_dflash2_linear_swiglu_small_t_launch), 40},
        {"mma_r32_c64_k128", make(&detail::w8_dflash2_linear_swiglu_mma_r32_c64_k128_launch), 0},
        {"mma_r64_c64_k128", make(&detail::w8_dflash2_linear_swiglu_mma_r64_c64_k128_launch), 0},
        {"mma_r64_c80_k128", make(&detail::w8_dflash2_linear_swiglu_mma_r64_c80_k128_launch), 0},
        {"mma_r64_c96_k128", make(&detail::w8_dflash2_linear_swiglu_mma_r64_c96_k128_launch), 0},
        // What DFlash2MmaR64C128 resolves to: the shared non-DFlash2 kernel, not a k128 variant.
        {"mma_r64_c128", make(&detail::w8_linear_swiglu_mma_r64_c128_launch), 0},
    };

    ninfer::bench::SweepOptions options = base;
    options.title                       = "w8 dflash2 linear_swiglu (27B)";
    std::string routed;
    options.routed_name = [&routed](std::int32_t tokens) -> const char* {
        const detail::W8LinearSwiGluProblem problem{kGateUpRows, kOutputRows, kHidden, kHidden,
                                                    tokens};
        routed = detail::w8_linear_swiglu_schedule_name(
            detail::w8_linear_swiglu_resolve_plan(problem).schedule);
        return routed.c_str();
    };
    options.public_op = [&](std::int32_t tokens, cudaStream_t stream) {
        Tensor x(input.p, DType::BF16, {kHidden, tokens});
        Tensor out(output.p, DType::BF16, {kOutputRows, tokens});
        detail::w8_linear_swiglu_dispatch(x, packed.weight, out, stream);
    };
    ninfer::bench::run_sweep(options, schedules);
    std::printf("\n");
}
